...
def math_extraction(image_path) -> dict:
    ...
    if response[0:2] == "No":
        return []
    prompt = f"""
        You are a helpful assistant helping people organize a piece of latex code. You will read some piece of latex source code. Identify different lines of latex code with each line representing a single formula or equation. Make sure each line can compile by themselves. Organize the latex code in the following way: Codes: ["line1", "line2", ...]. Do not give any extra outputs.
        Latex codes: {response}
    """
    response = gpt_api_call(prompt, 0.0, api_key, None)
    ...
    prompt = f"""
        You are a helpful assistant helping people understanding latex codes. You will read a list of latex source codes. Each line of code represent a complete formula, equation or theorem. Based on the content and the context, summarize what each line of code is trying to show. Organize your answer in the form Interpretations: ["Interpretation of line1", "Interpretation of line2", ...]. Make sure each line have a interpretation. Do not give any extra contents.
        Latex code list: {raw_response}
    """
    response = gpt_api_call(prompt, 0.0, api_key, None)
